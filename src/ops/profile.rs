...
fn render_shell_nix(shell: &MergedShell, platform: Platform) -> String {
    let mut lines = vec![
        ...
        "  programs.bash.package = pkgs.bashInteractive;".to_string(),
    ];
...
fn apply_kitty(
    ...
    // macOS GUI launches Kitty from launchd's sparse PATH, where a bare
    // `shell bash` resolves to Apple's Bash 3.2. Profiles generated by Nex use
    // home-manager's Bash package and may contain Bash-4+ syntax, so pin bare
    // Kitty bash launches to the per-user Nix profile before copying files.
    let nix_bash = std::env::var("USER").ok().map(|user| {
...
fn rewrite_kitty_bare_bash(dir: &Path, bash: &Path) -> Result<()> {
    if !bash.exists() {
        return Ok(());
    }
    ...
    for entry in std::fs::read_dir(dir).with_context(|| format!("reading {}", dir.display()))? {
        let path = entry?.path();
        if path.is_dir() {
            rewrite_kitty_bare_bash(&path, bash)?;
            continue;
        }
